...
DOCKER_COMPOSE_PHP_FPM_EXEC = ${DOCKER_COMPOSE} exec -u www-data php-fpm-publisher
...
cache:
	docker-compose -f ./docker/docker-compose.yml exec -u www-data php-fpm-publisher bin/console cache:clear
	docker-compose -f ./docker/docker-compose.yml exec -u www-data php-fpm-publisher bin/console cache:clear --env=test
...
db_migrate:
	${DOCKER_COMPOSE} exec -u www-data php-fpm-publisher bin/console doctrine:migrations:migrate --no-interaction
db_diff:
	${DOCKER_COMPOSE} exec -u www-data php-fpm-publisher bin/console doctrine:migrations:diff --no-interaction
db_drop:
	docker-compose -f ./docker/docker-compose.yml exec -u www-data php-fpm-publisher bin/console doctrine:schema:drop --force
